...
class AgentCallback(BaseCallbackHandler):
    """Base callback handler for agent execution."""
...
class SQLAgentCallback(AgentCallback):
    """Callback handler specifically for SQL agent execution."""
...
class RAGCallback(AgentCallback):
    """Callback handler specifically for RAG execution."""
    
    def on_llm_end(self, response: LLMResult, **kwargs) -> None:
        """Called when LLM completes a generation."""
        print("\nDocument retrieval and answer generation completed.")
